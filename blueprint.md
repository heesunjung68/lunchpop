...
A simple web application that randomly generates a lunch menu suggestion from a predefined list. It also includes an affiliate inquiry form using Formspree.
...
*   `index.html`: The main HTML file. It will contain the basic page structure, including a button and placeholders for the menu name and image, as well as the affiliate inquiry form.
*   `main.js`: Contains the JavaScript logic for generating random menu items and updating the display.
*   `style.css`: Contains the CSS for styling the page, including styles for the lunch menu generator and the affiliate inquiry form.
